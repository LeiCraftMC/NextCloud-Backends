...
# Iterate over possible backend IDs
for i in {0..99}; do  # Adjust range as needed
    NC_DOMAIN_VAR="NC_DOMAIN_$i"
    SIGNALING_SECRET_VAR="SIGNALING_SECRET_$i"
    
    # Check if both variables are set
    if [[ -n "${!NC_DOMAIN_VAR}" && -n "${!SIGNALING_SECRET_VAR}" ]]; then
        BACKEND_NAME="backend-$i"
        BACKENDS+=("$BACKEND_NAME")
        
        # Append backend config to string
        BACKEND_CONFIG+="\n[$BACKEND_NAME]\n"
        BACKEND_CONFIG+="url = https://${!NC_DOMAIN_VAR}\n"
        BACKEND_CONFIG+="secret = ${!SIGNALING_SECRET_VAR}\n"
        BACKEND_CONFIG+="maxstreambitrate = ${TALK_MAX_STREAM_BITRATE}\n"
        BACKEND_CONFIG+="maxscreenbitrate = ${TALK_MAX_SCREEN_BITRATE}\n"
    fi
done

# Add the backends list if at least one backend is configured
